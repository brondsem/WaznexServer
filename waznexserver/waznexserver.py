...
def index():
    ...
    if grid is not None and grid.level == models.IMAGELEVEL_GRID:
        cell_list = (
            db.session.query(models.GridCell)
    ...
    return render_template(
        "index.html", grid=grid, pretty_dt_format=app.config["PRETTY_DT_FORMAT"]
    )
...
def favicon():
    ...
    return send_from_directory(
        os.path.join(app.root_path, "static", "favicon"), filename
    )
...
def show_colview(grid_item_id, col_num):
    ...
    cells = (
        ...
        .filter(or_(models.GridCell.col == 0, models.GridCell.col == col_num))
        .order_by("row")
        .order_by("col")
        .all()
    )
