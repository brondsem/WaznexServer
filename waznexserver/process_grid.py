...
def run_basic_transforms(grid_image):
    try:
        ...
        print(
            "Error while performing basic transforms on {}".format(grid_image.filename)
        )
        traceback.print_exc()
        return False
...
def run_gridsplitter(grid_image):
    ...
    cells = [
        c for c in os.listdir(grid_dir) if c.startswith(config.GRIDSPLITTER_CELL_PREFIX)
    ]
    for cell in cells:
...
def verify_gridsplitter(grid_image):
    ...
    for cell in cells:
        ...
        if (
            col < 0
            or col > config.GRIDSPLITTER_MAX_COLS
            or row < 0
            or row > config.GRIDSPLITTER_MAX_ROWS
        ):
...
def process_new_images():
    new_grids = (
        db.session.query(models.GridItem)
