...
def FindDotsInRanges(dots, tlDotIndex, angles, tolerance, minDists):
    ...
    angleRanges = [
        (angles[a] - tolerance[a], angles[a] + tolerance[a])
...
def SliceSquares(imageOriginal, channel, drawDebuggingGrid=None):
    ...
    if drawDebuggingGrid:
        for d in dots:
            drawDebuggingGrid.ellipse(
                (d[0] - 10, d[1] - 10, d[0] + 10, d[1] + 10), fill="red"
            )
    ...
    if len(dots) > 0:
        ...
        if len(dots) < len(topColumnDots) * len(leftRowDots):
            ...
            bottomColumnDots = FindDotsInLine(
                dots, leftRowDots[len(leftRowDots) - 1], True
            )
            rightRowDots = FindDotsInLine(
                dots, topColumnDots[len(topColumnDots) - 1], False
            )

            # Find lengths of top, right, bottom, and left edges so we can properly weight averages
            topVec = (
                dots[topColumnDots[len(topColumnDots) - 1]][0]
                - dots[topColumnDots[0]][0],
                dots[topColumnDots[len(topColumnDots) - 1]][1]
                - dots[topColumnDots[0]][1],
            )
            ...
            bottomVec = (
                dots[bottomColumnDots[len(bottomColumnDots) - 1]][0]
                - dots[bottomColumnDots[0]][0],
                dots[bottomColumnDots[len(bottomColumnDots) - 1]][1]
                - dots[bottomColumnDots[0]][1],
            )
            ...
            for row, leftDot in enumerate(leftRowDots[1:-1]):
                try:  # Generate a row-wide vector
                    ...
                    rowVec = (
                        dots[rightDot][0] - dots[leftDot][0],
                        dots[rightDot][1] - dots[leftDot][1],
                    )
                except Exception:
                ...
                for col, topDot in enumerate(topColumnDots[1:-1]):
                    ...
                    colVec = (
                        dots[bottomDot][0] - dots[topDot][0],
                        dots[bottomDot][1] - dots[topDot][1],
                    )

                    # Figure out how far along the top and bottom sides we are
                    topLen = hypot(
                        dots[topDot][0] - dots[topColumnDots[0]][0],
                        dots[topDot][1] - dots[topColumnDots[0]][1],
                    )
                    ...
                    ratioWithinRow = (topRatio * amountOfTop) + (
                        bottomRatio * (1 - amountOfTop)
                    )
...
if __name__ == "__main__":
    ...
    for y in range(0, len(squares)):
        for x in range(0, len(squares[y])):
            squares[y][x].save(
                outputDir + sep + "out-" + str(x) + "-" + str(y) + ".jpg"
            )
